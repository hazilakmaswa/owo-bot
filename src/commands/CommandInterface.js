 ...
 * Original: https://github.com/hazilakmaswa/Discord-OwO-Bot (source files retained attribution)
 * Adapted for owo-bot, storage switched from MySQL to croxydb.
 */
...
class CommandInterface {
	...
	async execute(params) {
			...
			for (let i in this.permissions) {
				if (!channelPerms.has(this.permissions[i])) {
					if (channelPerms.has('sendMessages') && channelPerms.has('readMessages'))
						params.errorMsg(', please re-invite the bot, or contact your server admin!', 4000);
					params.logger.incr('noperms', 1, { permission: this.permissions })
					return;
				}
			}
		}
		...
		await this.executeCommand.bind(this)(params);
	}
}
